    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $edit_btn = '<a href="' . route('admin.coupons.edit', $query->id) .
                    '" class="btn btn-primary"><i class="far fa-edit"></i></a>';
                $delete_btn = '<a href="' . route('admin.coupons.destroy', $query->id) .
                    '" class="btn btn-danger ml-2 delete-item"><i class="far fa-trash-alt"></i></a>';

                return $edit_btn . $delete_btn;
            })
            ->addColumn('discount', function ($query) {
                return match ($query->discount_type) {
                    2 => GeneralSetting::query()->first()->currency_icon . $query->discount,
                    default => $query->discount . '%',
                };
            })
