    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                return '<a href="' . route('admin.flash-sale.destroy', $query->id) .
                    '" class="btn btn-danger ml-2 delete-item"><i class="far fa-trash-alt"></i></a>';
            })
            ->addColumn('product', function ($query) {
                return '<a href="' . route('admin.products.edit', $query->product->id) .
                    '">' . $query->product->name . '</a>';
            })
            ->addColumn('show_at_home', function ($query) {
                return '<label class="custom-switch mt-2">
                        <input type="checkbox" name="show_at_home" class="custom-switch-input change-show-at-home" ' .
                    ($query->show_at_home == 1 ? 'checked' : '') . ' data-id="' . $query->id . '">
            ...
            ->rawColumns(['action', 'show_at_home', 'status', 'product'])
            ->setRowId('id');
    }
