    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $editBtn = "<a href='" . route('admin.withdraw-method.edit', $query->id) .
                    "' class='btn btn-primary'><i class='far fa-edit'></i></a>";
                $deleteBtn = "<a href='" . route('admin.withdraw-method.destroy', $query->id) .
                    "' class='btn btn-danger ml-2 delete-item'><i class='far fa-trash-alt'></i></a>";

                return $editBtn . $deleteBtn;
            })
            ->addColumn('minimum_amount', function ($query) {
                return GeneralSetting::query()->first()->currency_icon .
                    number_format($query->minimum_amount, 2);
            })
            ->addColumn('maximum_amount', function ($query) {
                return GeneralSetting::query()->first()->currency_icon .
                    number_format($query->maximum_amount, 2);
            })
