                ...
                <div class="relative_contect d-flex">
                    ...
                    <ul class="wsus_menu_cat_item show_home toggle_menu">
                        @foreach( $categories as $category )
                            <li><a class="{{ count($category->subcategories) > 0
                                    ? 'wsus__droap_arrow' : '' }}"
                                        ...
                                        @foreach( $category->subcategories as $subcategory )
                                            <li><a href="{{ route('products.index',
                                                ['subcategory' => $subcategory->slug]) }}">{{ $subcategory->name
                                                    }} <i class="{{ count($subcategory->childCategories) > 0
                    ...
                    <ul class="wsus__menu_item">
                        ...
                        <li><a class="{{ setActive(['about']) }}" href="{{ route('about') }}">
                                about</a></li>
                        <li><a class="{{ setActive(['vendors.index']) }}" href="{{ route('vendors.index') }}">
                                vendor</a></li>
                        <li><a class="{{ setActive(['flash-sale']) }}" href="{{ route('flash-sale') }}">
                                flash sale</a></li>
                        <li><a class="{{ setActive(['blog']) }}" href="{{ route('blog') }}">blog</a></li>
                        <li><a class="{{ setActive(['contact']) }}" href="{{ route('contact') }}">
                                contact</a></li>
                    </ul>
...
<section id="wsus__mobile_menu">
    ...
    <ul class="wsus__mobile_menu_header_icon d-inline-flex">
        <li><a href="{{route('user.wishlist.index')}}">
                <i class="fal fa-heart"></i><span class="wishlist-count">
                    @if ( auth()->check() )
                        {{ \App\Models\Wishlist::query()->where('user_id', auth()->user()->id)->count() }}
                    @else
                        0
                    @endif
                </span>
            ...
            <div class="wsus__mobile_menu_main_menu">
                <div class="accordion accordion-flush" id="accordionFlushExample">
                    <ul class="wsus_mobile_menu_category">
                        @foreach( $categories as $category )
                            <li><a href="#" class="{{ count($category->subcategories) > 0
